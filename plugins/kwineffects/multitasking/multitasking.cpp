...
void MultitaskingEffect::onWindowClosed(QVariant winId, int index)
{
    ...
    for (int i = 0; i < winList.length(); i++) {
        KWin::EffectWindow *ceffectWindow = effects->findWindow(winList[i]);
        if (!ceffectWindow) {
            continue;
        }
        ceffectWindow->closeWindow();
...
void MultitaskingEffect::prePaintWindow(EffectWindow *w, WindowPrePaintData &data, int time)
{
    ...
    if (!(w->isDesktop() || w->isNotification() || isRelevantWithPresentWindows(w))) {
...
{
    return (!m_animations.isEmpty() || m_hideWindow) && !effects->isScreenLocked();
}
